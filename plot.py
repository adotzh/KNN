...
def plot_mnist(images, labels, N, M, figsize=(6, 6), random_seed=42, error_label=[], error=False):
    np.random.seed(random_seed)
    f, axarr = plt.subplots(N, M, figsize=figsize)
    indices = list(np.random.randint(images.shape[0], size=N*M))
    for i, indice in enumerate(indices):
        plt.subplot(N, M, i + 1)
        plt.imshow(images[indice].reshape(28,28), cmap='gray_r', interpolation='none')
        if error:
            plt.title("true:{}, pred: {}".format(labels[indice], error_label[indice]))
        else:
            plt.title("label:{}".format(labels[indice]))
        plt.tight_layout()
        plt.axis("off")

def plot_confusion_matrix(confusion_matrix, class_names, errors_only=False, figsize = (20,8), fontsize=16):
    """
